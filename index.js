...
io.on("connection", (socket) => {
  ...
  socket.on("friendRequest", ({ to }) => {
    ...
    io.to(to).emit("friendRequest", { from: socket.id, name: players[socket.id]?.name });
  });
  ...
  socket.on("friendAccept", ({ from }) => {
    ...
    io.to(socket.id).emit("friendAccepted", { id: from, name: players[from]?.name });
    io.to(from).emit("friendAccepted", { id: socket.id, name: players[socket.id]?.name });
  });
